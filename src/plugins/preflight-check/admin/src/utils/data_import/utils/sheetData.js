...
const category_of_employment = {
  ...
  function cleanResidenceSheetData (data, lookup, pk){
    const obj = {}
    data.forEach((row) => {
        const rowObj = {}
        ...
        obj[rowObj[pk]] ? obj[rowObj[pk]].participants.push(rowObj.participant_id)
        : obj[rowObj[pk]] = removeNullUndefined({
            participants: [rowObj.participant_id],
            residence_in_1977: rowObj.residence_in_1977,
            total_population: rowObj.total_population,
            median_household_income: rowObj.median_household_income,
        });
    })
    return obj;
  }
...
function cleanSheetObject (data, lookup, pk){
    ...
    data.forEach((row) => {
        ...
        if (Object.keys(removeNullUndefined(rowObj)).length > 1) obj[rowObj[pk]] = removeNullUndefined(rowObj);
    });
...
function cleanSheetObjectI (data, lookup, pk){
	...
	data.forEach((row) => {
		Object.entries(row).forEach(([key, value]) => {
			if (value === "yes") {
				obj[key] ? obj[key].participants.push(row["ID"]) : (obj[key] = {
                        participants: [row["ID"]],
                        [pk]: key,
                    });
			}
            ...
            if( value === 'for'){
                obj[key] ? obj[key].participants_for.push(row["ID"]) : (obj[key] = {
                    [pk]: key,
                    participants_for: [...obj[key]?.participants_for || [], row['ID']],
                    participants_against: [...obj[key]?.participants_against || []],
                    participants_spoke_for: [...obj[key]?.participants_spoke_for || []]
                });
            }
            if( value === 'against'){
                obj[key] ? obj[key].participants_against.push(row["ID"]) : (obj[key] = {
                    [pk]: key,
                    participants_for: [...obj[key]?.participants_for || []],
                    participants_against: [...obj[key]?.participants_against || [], row['ID']],
                    participants_spoke_for: [...obj[key]?.participants_spoke_for || []]
                });
            }
            if( value === 'spoke about with position unknown'){
                obj[key] ? obj[key].participants_spoke_for.push(row["ID"]) : (obj[key] = {
                    [pk]: key,
                    participants_for: [...obj[key]?.participants_for || []],
                    participants_against: [...obj[key]?.participants_against || []],
                    participants_spoke_for: [...obj[key]?.participants_spoke_for || [], row['ID']],
                });
            }
		});
	});
  ...
  function cleanSheetArray (data, lookup, pk){
    ...
    data.forEach((row) => {
        ...
        Object.entries(lookup).forEach(([key,value])=>{
            ...
            row[key] === 'yes' ? rowObj[value] = true
            : row[key] === 'present' ? rowObj[value] = true
            : row[key] === 'no' ? rowObj[value] = false 
            : reg.test(row[key]) ? rowObj[value] = parseInt(row[key]) 
            : rowObj[value] = row[key];
        })

        rowObj.category_of_employment ? rowObj.category_of_employment = category_of_employment[rowObj.category_of_employment] : null;

        if(Object.keys(removeNullUndefined(rowObj)).length > 1) array.push(removeNullUndefined(rowObj))
    })
    return array;
  }

  module.exports = {
    cleanResidenceSheetData,
    cleanSheetObject,
    cleanSheetObjectI,
    cleanSheetArray
  }